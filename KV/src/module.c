...
// To communicate with the user space, we use netlink sockets.
// The user space will use the netlink API to send messages to the kernel module.
// The kernel module will use the netlink API to send messages back to the user space.
...
// This function is the entry point for the netlink messages sent from user space.
// It processes the messages and performs the appropriate actions based on the command type.
static void kr_nl_recv(struct sk_buff *skb) {
    ...
    printk(KERN_INFO "kr_nl_recv: size=%d, type=%d\n", size, nlh->nlmsg_type);
