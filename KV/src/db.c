...
static void kr_db_init(KrDb* db)
{
    ...
    if (db->sb->magic == KR_SUPERBLOCK_MAGIC) {
        ...
        for (i = 0; i < db->sb->num_buckets; i++) {
            // i allocate a buffer for the KrDevice which is a block that contains many buckets . 
...
int kr_db_put (KrDb * db, KrSlice key, KrSlice val)
{
    ...
    kr_buf_markdirty(buf);
    kr_buf_unpin(buf);
    kr_buf_writeback(buf);
    return ret;
}

int kr_db_get (KrDb * db, KrSlice key, KrOutbuf* outbuf, u64* size, struct connection_context* cn)
{
    /* as above */
    u64 bkt_no = kr_get_bucket_num(db->sb, key);
    KrBuf* buf = kr_buf_read(db->dev, kr_bucket_block(bkt_no));
    ...
    vec.iov_base = tmp.data;
    vec.iov_len = tmp.size;
...
EXPORT_SYMBOL(kr_db_open);
EXPORT_SYMBOL(kr_db_from_id);
EXPORT_SYMBOL(kr_db_close);
EXPORT_SYMBOL(kr_db_put);
EXPORT_SYMBOL(kr_db_get);
