...
typedef struct __attribute__((packed)) KrBucketHdr {
    u16 data_off; /* offset to space used from bottom */ // where the data starts
...
// this function finds a key in the bucket and returns its KrTupleInfo
// if the key is not found, it returns NULL
// it searches backwards so that if a key has been updated, we find the most recent version
...
// this function defragments the bucket by moving all the key/value data to the end of the bucket
// it updates the offsets in the KrTupleInfo array accordingly
// reorganizing the data in the bucket to reclaim space
 ...
 */
 ...
 // this function deletes a key from the bucket
 // it finds the key in the bucket and removes its KrTupleInfo from the array
