...
typedef struct KrDevice {
    size_t bufcnt; // nbre actuel de buffers
    size_t maxbufs; // nbre max dans le cache 
    struct KrBuf** bufhash; /* maps buf IDs (disk locations) to KrBufs */ // permet de retrouver un buffer par son ID

    /* linux implementation */
    struct block_device* bdev; // what is this structure ?
    ...
    struct file *bdev_file; // fichier spécial associé au périphérique de bloc afin de manipuler bdev comme fichier . 
} KrDevice;
